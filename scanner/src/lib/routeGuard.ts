/**
 * Pure route-guard decisions for the gated SPA surfaces (`#dashboard`,
 * `#admin`). Kept side-effect-free so the redirect policy is unit-testable
 ...
 *
 * - `#dashboard`: anonymous visitors are bounced to `#login`. While the session
 *   is still `loading`, no decision is made (return `null`) so a brief load does
