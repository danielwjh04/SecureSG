...
- Produces: a private helper `bindGuardIdentity(payload, ctx, dbBound)` in `guard.ts` that returns the `PreToolUsePayload` used for ALL binding (action hash, cache key, ticket). For a `guard_device` caller it sets `device_id` from the authenticated credential (`ctx.deviceId`) and `provider` from the credential integration name (`payload.provider ?? ctx.integration`, preserving current behavior); for every other caller it DELETES any client-asserted `device_id`. Note `ctx.integration` is the integration NAME (maps to `provider`), not the semver `integration_version`; leave the client-supplied `integration_version` untouched (it is a self-claim that only scopes cache/ticket reuse and is never honored for a non-device caller). This replaces the inline ternary at lines 161-168.
...
- [ ] **Step 3: Implement.**
  - Add `bindGuardIdentity`. For `dbBound && ctx.credentialKind === 'guard_device'`: shallow-copy payload, set `device_id = ctx.deviceId` and `provider = payload.provider ?? ctx.integration`. Otherwise: shallow-copy and `delete` the client-asserted `device_id`. Use it to produce `guardPayload`.
  - The existing tests `issues and accepts a valid signed allow ticket` and `ignores a signed allow ticket when the action changes` (currently anonymous, `db===null`) MUST be migrated to a `guard_device` caller: use the `fixture()` + `createFreeUser` + `guardCredentialFor` seams from the second describe block (mint a credential, send `Authorization: Bearer <credential>`, pass `env` with `DB`). After this change those flows only work for an authenticated device. Run the whole `guard.test.ts` and migrate any other test that relied on anonymous ticket issue/honor.
